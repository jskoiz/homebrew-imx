...
while IFS=$'\t' read -r target url sha; do
  ...
  grep -Fx 'format=PPM width=2 height=1 channels=RGB depth=8' "$smoke_dir/identify-ppm.txt"
  ...
  grep -Fx 'format=QOI width=2 height=1 channels=RGBA depth=8' "$smoke_dir/identify-qoi.txt"
  ...
  grep -Fx 'format=FARBFELD width=2 height=1 channels=RGBA depth=16' "$smoke_dir/identify-farbfeld.txt"
done <"$records_file"
