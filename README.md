...
The `imx` formula installs the published IMX `v0.9.0` release archive for the
...
No Homebrew/core submission, macOS v0.9.0 tap support, Windows archive,
...
the current v0.9.0 tap formula.
...
Linux arm64 tap support is generated from the published v0.9.0 `SHA256SUMS` and
...
and checks deterministic PPM/JPEG same-format rewrites. The tap archive
verifier additionally smokes FARBFELD, JPEG, QOI, PBM, PGM, PNG, PPM, 16-bit
PGM/PNG/PPM outputs, same-format rewrites, and exact prefix identify/transcode
forms.
...
```sh
scripts/update-imx-formula.sh v0.9.0
```
