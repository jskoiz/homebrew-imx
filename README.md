...
The `imx` formula installs the published IMX `v0.10.0` release archive for the
...
No Homebrew/core submission, macOS v0.10.0 tap support, Windows archive,
crates.io package, APNG/indexed/low-bit PNG, progressive/CMYK/high-depth JPEG,
EXIF writing or preservation, GPS handling, color-management/profile transforms,
MagickCore/MagickWand API, or full ImageMagick CLI compatibility is claimed by
the current v0.10.0 tap formula.
...
Linux arm64 tap support is generated from the published v0.10.0 `SHA256SUMS` and
...
checks JPEG EXIF Orientation 6 identify/transcode normalization, and checks
deterministic PPM/JPEG same-format rewrites. The tap archive verifier
additionally smokes FARBFELD, JPEG, QOI, PBM, PGM, PNG, PPM, 16-bit
PGM/PNG/PPM outputs, same-format rewrites, orientation normalization, and exact
...
```sh
scripts/update-imx-formula.sh v0.10.0
```
