...
The `imx` formula installs the published IMX `v0.14.0` release archive for the
...
No Homebrew/core submission, macOS v0.14.0 tap support, Windows archive,
...
claimed by the current v0.14.0 tap formula.
...
Linux arm64 tap support is generated from the published v0.14.0 `SHA256SUMS` and
...
PPM/JPEG same-format rewrites, plus the v0.13 exact resize command and v0.14
resize-fit command. The tap
...
progressive JPEG input, v0.12 intake paths, v0.13 resize, v0.14 resize-fit, and
exact prefix identify/transcode forms.
...
```sh
scripts/update-imx-formula.sh v0.14.0
```
